...
@push('script-page')
    ...
    <script>
        ...
        $(document).ready(function() {
            var d_id = $('#selected_department').val();
            getEmployee(d_id);
        });
        ...
        function getEmployee(did) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(key, value) {
                        if(key==selected_employee)
                        {
                            $('.employee_id').append('<option value="' + key + '" selected>' + value +
                            '</option>');
                        }
                        else{
...
@section('action-button')
    <div class="float-end">
        ...
        <a href="{{ route('payroll.tire1.emp.report.export', [
            ...
            'employee' => !empty($_GET['employee_id']) ? $_GET['employee_id'] : 0,
        ]) }}"
            class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Tier 1 Returns (Employer’s Contribution)') }}"  style="margin-right: 8px;">
            <span class="btn-inner--icon"> <i class="ti ti-file-export"></i></span>
        </a>
        <a href="{{ route('payroll.tire2.emp.report.export', [
            'month' => isset($_GET['month']) ? $_GET['month'] : date('M, Y'),
            'branch' => !empty($_GET['branch']) ? $_GET['branch'] : 0,
            'department' => !empty($_GET['department_id']) ? $_GET['department_id'] : 0,
            'employee' => !empty($_GET['employee_id']) ? $_GET['employee_id'] : 0,
        ]) }}"
            class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Tier 2 Returns (Employer’s Contribution)') }}"  style="margin-right: 8px;">
            <span class="btn-inner--icon"> <i class="ti ti-file-export"></i></span>
        </a>
        <a href="{{ route('payroll.bank.advice.report.export', [
            'month' => isset($_GET['month']) ? $_GET['month'] : date('M, Y'),
            'branch' => !empty($_GET['branch']) ? $_GET['branch'] : 0,
            'department' => !empty($_GET['department_id']) ? $_GET['department_id'] : 0,
            'employee' => !empty($_GET['employee_id']) ? $_GET['employee_id'] : 0,
        ]) }}"
            class="btn btn-sm btn-primary" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Export Bank Advice Reports') }}"  style="margin-right: 8px;">
            <span class="btn-inner--icon"> <i class="ti ti-file-export"></i></span>
        </a>
                        ...
                        <div class="row align-items-center justify-content-end">
                            <div class="col-xl-11">
                                <div class="row">
                                    ...
                                    <div class="col-xl-2 col-lg-3 col-md-6 col-sm-12 col-12">
                                        <input type="hidden" id="selected_department" name="selected_department" value="{{isset($_GET['department_id']) ? $_GET['department_id'] : ''}}">
                                    ...
                                    <div class="col-xl-2 col-lg-3 col-md-6 col-sm-12 col-12">
                                        <input type="hidden" id="selected_employee" name="selected_employee" value="{{isset($_GET['employee_id']) ? $_GET['employee_id'] : ''}}">
                                        {{-- {{ Form::label('department', __('Department'), ['class' => 'form-label']) }}
                                        <div class="btn-box department_div" id="department_id">
                                            {{ Form::select('department', $department, isset($_GET['department']) ? $_GET['department'] : '', ['class' => 'form-control select department_id', 'id' => 'department_id']) }}
                                        </div> --}}

                                        <div class="form-icon-user" id="employee_div">
                                            {{ Form::label('department', __('Employee'), ['class' => 'form-label']) }}
                                            <select class="form-control select employee_id" name="employee_id"
                                                id="employee_id" placeholder="Select Employee">
                                            </select>
                                        </div>
            ...
            <div class="row mt-3">
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('Total Cash Emoluments') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['total_cash_emoluments']) }}</h7>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('SSF Contribution') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['ssf_Contribution']) }}</h7>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('Bonus Tax') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['bonus_tax']) }}</h7>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('Total Reliefs') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['total_reliefs']) }}</h7>
                    </div>
                </div>
            </div>
            <div class="row mt-3">
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('Chargeable Income') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['chargeable_income']) }}</h7>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('Tax Deductible ') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['tax_deductible']) }}</h7>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('Overtime Tax') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['overtime_tax']) }}</h7>
                    </div>
                </div>
                <div class="col">
                    <div class="card p-4 mb-4">
                        <h6 class="report-text gray-text mb-0">{{ __('Total Tax') }} :</h6>
                        <h7 class="report-text mb-0">{{ \Auth::user()->priceFormat($filterData['total_Tax']) }}</h7>
                    </div>
                </div>
            </div>
            <div class="row mt-3">
