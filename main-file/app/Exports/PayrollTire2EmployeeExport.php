...
class PayrollTire2EmployeeExport implements FromArray, WithHeadings, WithStyles, WithCustomStartCell, WithColumnWidths, WithEvents
{
    protected $month;
    protected $branch;
    ...
    public function __construct($data, $companyName, $month, $address)
    {
        ...
        foreach ($data as $key => $type) {
            $amount=(5.5 / 100) * $type->basic_salary;
            $formattedData[] = [
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                $event->sheet->getDelegate()->mergeCells('A1:D1'); // Merge A to D for COMPANY NAME
                $event->sheet->getDelegate()->mergeCells('A1:D2'); // Merge A to D for COMPANY NAME
    
                $event->sheet->getStyle('A1:A1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
    
                $event->sheet->getCell('A1')->setValue('For the Month of: ' . $this->month);
